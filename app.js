...
// Mengubah string ID admin menjadi array of numbers
            ...
            db.run(`CREATE TABLE IF NOT EXISTS Server (
              ...
              if (err) {
                ...
                db.all("PRAGMA table_info(Server)", (err, columns) => {
                  if (err) {
                    logger.error('Error getting table info for Server:', err.message);
                    return;
                  }
                  const hasSshEnabledColumn = columns.some(col => col.name === 'ssh_enabled');
                  if (!hasSshEnabledColumn) {
                    db.run("ALTER TABLE Server ADD COLUMN ssh_enabled INTEGER DEFAULT 1", (err) => {
                      if (err) logger.error('Error adding ssh_enabled column:', err.message);
                      else logger.info('✅ Added ssh_enabled column to Server table');
                    });
                  }
                });
              }
...
<b>╰──────────────────────────────╯</b>
<b>╭─────────── OS Tersedia  ──────────╮</b>
<b>┣[01] Debian 10 : Ubuntu 20.04</b> 
<b>┣[02] Debian 11 : Ubuntu 22.04</b>  
<b>┣[03] Debian 12 : Ubuntu 24.04</b>  
<b>╰──────────────────────────────╯</b>
<b>╭──────────────────────────────╮</b>
<b>┣📜 Catatan:</b>  
<b>┣• Sewa script dari kami bisa digunakan dengan</b>
<b>┣• bot auto order Mendukung bot pribadi atau publik.</b>
<b>┣• Cocok untuk usaha jualan akun tanpa</b>
<b>┣• perlu menunggu admin online.</b>
<b>┣
...
keyboard.push([{ text: '💰 Top Up', callback_data: 'menu_topup' }]);
  ...
  try {
    // Selalu kirim menu sebagai pesan baru karena menu lama sudah dihapus
    ...
    // Simpan ID dari menu yang baru dikirim agar bisa dihapus nanti
    ...
    for (const row of rows) {
      try {
        ...
        if (reply && reply.message_id) {
          // copyMessage -> return { message_id: number }
          sent = await bot.telegram.copyMessage(row.user_id, ctx.chat.id, reply.message_id);
        } else if (inputText) {
          // sendMessage -> return Message object (ada .message_id)
        ...
        // Antisipasi bentuk return: copyMessage mengembalikan object kecil,
        // sendMessage mengembalikan object Message penuh.
        const messageIdToPin = sent?.message_id || sent;
        if (messageIdToPin) {
          await bot.telegram.pinChatMessage(row.user_id, messageIdToPin, {
            disable_notification: false // set true kalau mau tanpa notif pin
          });
          // TIDAK memanggil unpinAllChatMessages -> sematan akan menumpuk
        }

        success++;
        logger.info(`✅ Broadcast + pin sukses ke ${row.user_id}`);
...
bot.action('menu_topup', async (ctx) => {
  ...
  if (config.topup_saldo)
    keyboard.push([{ text: "❇️ Top Up QRIS Order Kuota", callback_data: "topup_saldo" }]);
  if (config.topup_saweria)
    keyboard.push([{ text: "❇️ Top Up QRIS Saweria", callback_data: "topup_saweria" }]);
  keyboard.push([{ text: "🔙 Kembali", callback_data: "send_main_menu" }]);

  // ...generate messageText sesuai menu_topup lama...
  const messageText = `
🏷️ Silahkan Pilih Opsi Top Up Di Bawah ini:`;
...
bot.command('editharga', async (ctx) => {
  ...
  const args = ctx.message.text.split(' ');
  if (args.length !== 3) {
      return ctx.reply('⚠️ Format salah. Gunakan: `/editharga <domain> <harga_bulanan>`', { parse_mode: 'Markdown' });
  }

  const [domain, hargaBulananStr] = args.slice(1);
  const hargaBulanan = parseInt(hargaBulananStr, 10);

  if (isNaN(hargaBulanan) || hargaBulanan <= 0) {
      return ctx.reply('⚠️ `harga_bulanan` harus berupa angka positif.', { parse_mode: 'Markdown' });
  }

  // LANGSUNG SIMPAN HARGA BULANAN
  db.run("UPDATE Server SET harga = ? WHERE domain = ?", [hargaBulanan, domain], function(err) {
      if (err) {
      ...
      ctx.reply(`✅ Harga server \`${domain}\` berhasil diubah menjadi *Rp${hargaBulanan.toLocaleString('id-ID')}* per bulan.`, { parse_mode: 'Markdown' });
  });
});
...
async function handleServiceAction(ctx, action) {
    ...
    if (action === 'trial' || action === 'create' || action === 'renew') {
        sshServerAvailable = await new Promise((resolve) => {
            db.get('SELECT COUNT(*) AS count FROM Server WHERE ssh_enabled = 1', [], (err, row) => {
                if (err) {
                    logger.error('❌ Gagal memeriksa server SSH:', err.message);
                    return resolve(false);
                }
                resolve(row.count > 0);
            });
...
// Handler untuk tombol "Reset Total Create"
bot.action('admin_reset_total_create', async (ctx) => {
    try {
        await ctx.answerCbQuery();
        db.all('SELECT id, nama_server FROM Server', [], (err, servers) => {
            ...
            const keyboard = servers.map(server => ([{ text: server.nama_server, callback_data: `reset_totalcreate_${server.id}` }]));
            ctx.reply('🔄 Pilih server untuk mereset Total Create Akun menjadi 0:', {
                reply_markup: { inline_keyboard: keyboard }
            });
        });
...
// Handler lanjutan SETELAH admin memilih server untuk di-reset
bot.action(/^reset_totalcreate_(\d+)$/, async (ctx) => {
    const serverId = ctx.match[1];
    db.run('UPDATE Server SET total_create_akun = 0 WHERE id = ?', [serverId], function (err) {
        if (err) {
            ctx.answerCbQuery('❌ Gagal mereset.', { show_alert: true });
            return logger.error(err.message);
        }
        ctx.editMessageText(`✅ Total create akun untuk server berhasil direset menjadi 0.`);
        ctx.answerCbQuery('✅ Berhasil direset!');
    });
});

// Handler untuk tombol "Kurangi Total Create"
bot.action('admin_kurangi_total_create', async (ctx) => {
    try {
        await ctx.answerCbQuery();
        db.all('SELECT id, nama_server FROM Server', [], (err, servers) => {
            if (err || servers.length === 0) {
            ...
            const keyboard = servers.map(server => ([{ text: server.nama_server, callback_data: `kurangi_totalcreate_select_${server.id}` }]));
            ctx.reply('➖ Pilih server untuk mengurangi Total Create Akun:', {
                reply_markup: { inline_keyboard: keyboard }
            });
        });
    } catch (e) { console.error(e); }
...
// Handler lanjutan SETELAH admin memilih server
bot.action(/^kurangi_totalcreate_select_(\d+)$/, async (ctx) => {
    const serverId = ctx.match[1];
    const userId = ctx.from.id;
    userState[userId] = { step: 'input_kurangi_total_create', serverId: serverId };
    try {
        await ctx.editMessageText('🔢 Masukkan jumlah yang ingin dikurangkan (misal: 5):');
    } catch(e){}
});

bot.action('kelola_tombol_ssh', async (ctx) => {
    try {
        await ctx.answerCbQuery();
        db.all('SELECT id, nama_server, ssh_enabled FROM Server', [], async (err, servers) => {
            if (err || servers.length === 0) {
                // Jika tidak ada server, kirim pesan baru karena tidak ada pesan untuk diedit
                return ctx.reply('⚠️ Tidak ada server yang tersedia.', {
                    reply_markup: {
                        inline_keyboard: [
            ...
            const keyboard = servers.map(server => {
                const status = server.ssh_enabled ? '✅ AKTIF' : '❌ NONAKTIF';
                return [{
                    text: `${server.nama_server} - ${status}`,
                    callback_data: `ubah_status_ssh_${server.id}`
                }];
            });

            keyboard.push([{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]);

            // Gunakan editMessageText untuk mengedit pesan yang ada
            await ctx.editMessageText('⚙️ Pilih server untuk mengubah status tombol SSH:', {
                reply_markup: { inline_keyboard: keyboard }
            });
        });
    } catch (e) {
        // Fallback jika gagal mengedit pesan (misalnya, pesan sudah terlalu lama)
        logger.error('❌ Gagal mengedit pesan "kelola_tombol_ssh":', e.message);
        await ctx.reply('⚙️ Pilih server untuk mengubah status tombol SSH:', {
            reply_markup: { inline_keyboard: keyboard }
        });
    }
});

bot.action(/^ubah_status_ssh_(\d+)$/, async (ctx) => {
    const serverId = ctx.match[1];
    await ctx.answerCbQuery();

    db.get('SELECT ssh_enabled FROM Server WHERE id = ?', [serverId], (err, row) => {
        if (err || !row) {
            return ctx.reply('❌ Gagal mengambil status server.');
        }

        const newStatus = row.ssh_enabled === 1 ? 0 : 1;
        const statusText = newStatus === 1 ? 'AKTIF' : 'NONAKTIF';

        db.run('UPDATE Server SET ssh_enabled = ? WHERE id = ?', [newStatus, serverId], (err) => {
            if (err) {
                logger.error('❌ Gagal update status SSH:', err.message);
                return ctx.reply('❌ Gagal mengubah status tombol SSH.');
            }
            
            ctx.reply(`✅ Tombol SSH untuk server berhasil diubah menjadi *${statusText}*.`, { parse_mode: 'Markdown' });
        });
    });
});
...
const showSewaScript = await new Promise((resolve) => {
    ...
    const adminKeyboard = [
        ...
        [{ text: '🔄 Reset Total Create', callback_data: 'admin_reset_total_create' }, { text: '➖ Kurangi Total Create', callback_data: 'admin_kurangi_total_create' }],
        ...
        [{ text: '💰 Rekap TopUp', callback_data: 'rekap_topup' }],
        [{ text: '👥 List Reseller', callback_data: 'listreseller' }],
        [{ text: '⚙️ Kelola Tombol SSH', callback_data: 'kelola_tombol_ssh' }],
        [{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]
        
    ];

    const messageText = `
🏷️ Silahkan Pilih Opsi Di Bawah ini:`;
...
bot.action('send_main_menu', async (ctx) => {
  try {
    await ctx.answerCbQuery();
    // Langsung panggil fungsi sendMainMenu untuk menangani semuanya
    await sendMainMenu(ctx);
  } catch (error) {
...
async function startSelectServer(ctx, action, type, page = 0) {
    try {
        ...
        // Tentukan query SQL berdasarkan jenis akun (hanya SSH yang perlu difilter)
        const query = (type === 'ssh') ?
            'SELECT * FROM Server WHERE ssh_enabled = 1' :
            'SELECT * FROM Server';
        ...
        db.all(query, [], (err, servers) => {
            ...
            if (servers.length === 0) {
                ...
                // Berikan pesan spesifik jika tidak ada server SSH yang tersedia
                if (type === 'ssh') {
                    return ctx.reply('⚠️ *PERHATIAN!* Tidak ada server SSH yang tersedia saat ini. Silahkan coba layanan lain.', { parse_mode: 'Markdown' });
                }
                return ctx.reply('⚠️ *PERHATIAN!* Tidak ada server yang tersedia saat ini. Coba lagi nanti!', { parse_mode: 'Markdown' });
            }
            ...
            const serverList = currentServers.map(server => {
                let hargaBulananTampilan = server.harga; // << HARGA SEKARANG SUDAH BULANAN
                // Terapkan diskon untuk tampilan jika user adalah reseller
                if (userRole === 'reseller' && resellerDiscount > 0) {
                    hargaBulananTampilan = Math.floor(server.harga * (100 - resellerDiscount) / 100);
                }
...
bot.action(/^(create|renew|trial)_username_(vmess|vless|trojan|shadowsocks|ssh)_(.+)$/, async (ctx) => {
  ...
  db.get('SELECT batas_create_akun, total_create_akun FROM Server WHERE id = ?', [serverId], (err, server) => {
    ...
    db.get('SELECT batas_create_akun, total_create_akun FROM Server WHERE id = ?', [serverId], async (err, server) => {
      ...
      if (action === 'create' && total_create_akun >= batas_create_akun) {
...
bot.action(/^(create|renew)_duration_(vmess|vless|trojan|shadowsocks|ssh)_(\d+)_(\d+)$/, async (ctx) => {
    const userId = ctx.from.id;
    const state = userState[userId];

    if (!state || !state.username) {
        try { await ctx.deleteMessage(); } catch(e) {}
        return ctx.reply('❌ Sesi Anda telah berakhir. Silakan ulangi dari awal.', { parse_mode: 'Markdown' });
    }
    
    try { await ctx.deleteMessage(); } catch(e) {}

    const [, action, type, serverId, durationMonths] = ctx.match;
    const exp = parseInt(durationMonths) * 30;

    db.get('SELECT quota, iplimit, harga, nama_server, domain FROM Server WHERE id = ?', [serverId], async (err, server) => {
        if (err) {
            logger.error('⚠️ Error fetching server details:', err.message);
        ...
        // PERBAIKAN LOGIKA HARGA: harga bulanan * jumlah bulan
        let totalHarga = server.harga * parseInt(durationMonths);
        ...
        if (userRole === 'reseller') {
            const resellerDiscount = await new Promise((resolve) => {
                db.get('SELECT discount_percent FROM reseller_config WHERE id = 1', (err, row) => {
                    resolve(row ? row.discount_percent : 0);
                });
            });
        ...
        db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], async (err, user) => {
            ...
            if (user.saldo < totalHarga) {
                delete userState[userId];
                return ctx.reply(`❌ *Saldo Anda tidak mencukupi. Saldo Anda: Rp${user.saldo.toLocaleString('id-ID')}, Harga: Rp${totalHarga.toLocaleString('id-ID')}*`, { parse_mode: 'Markdown' });
            }
            ...
            try {
                if (action === 'create') {
                    ...
                    switch (type) {
                        case 'vmess': msg = await createvmess(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'vless': msg = await createvless(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'trojan': msg = await createtrojan(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'shadowsocks': msg = await createshadowsocks(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'ssh': msg = await createssh(state.username, state.password, exp, server.iplimit, serverId); break;
                    }
                } else if (action === 'renew') {
                    actionTypeLabel = 'Perpanjang Akun';
                     switch (type) {
                        case 'vmess': msg = await renewvmess(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'vless': msg = await renewvless(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'trojan': msg = await renewtrojan(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'shadowsocks': msg = await renewshadowsocks(state.username, exp, server.quota, server.iplimit, serverId); break;
                        case 'ssh': msg = await renewssh(state.username, exp, server.iplimit, serverId); break;
                    }
                }
                ...
                if (msg && typeof msg === 'string' && !msg.toLowerCase().includes('gagal') && !msg.toLowerCase().includes('error')) {
                    ...
                    msg = msg || `❌ Gagal ${actionTypeLabel} akun. Mohon coba lagi.`;
                }

            } catch (e) {
                logger.error(`Error saat memanggil fungsi ${actionTypeLabel} akun ${type}:`, e.message);
                msg = '❌ Terjadi kesalahan internal saat memproses akun Anda.';
            } finally {
                clearInterval(intervalId);
...
*AKUN MUNGKIN SUDAH TERBUAT TAPI SALDO BELUM TERPOTONG!*`, { parse_mode: 'Markdown' }).catch(e => logger.error("Gagal kirim notif darurat:", e.message));
            ...
            if (action === 'create') {
                db.run('UPDATE Server SET total_create_akun = total_create_akun + 1 WHERE id = ?', [serverId], (err) => { if (err) logger.error('Gagal update total create:', err.message); });
            }

            db.run(`INSERT INTO log_penjualan (user_id, username, nama_server, tipe_akun, harga, masa_aktif_hari, waktu_transaksi, action_type, user_role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [userId, ctx.from.username || '', server.nama_server, type, totalHarga, exp, new Date().toISOString(), action, userRole], (err) => { if(err) logger.warn('Gagal mencatat log penjualan:', err.message); });
            
            await afterAccountTransaction({ userId, username: ctx.from.username, produk: type.toUpperCase(), serverId, jenis: actionTypeLabel, durasi: exp, accountUsername: state.username });
            await ctx.reply(msg, { parse_mode: 'Markdown' });
            delete userState[userId];
        });
    });
...
🎁 Bonus: ${persen}%`, {
    ...
    if (state && state.step.startsWith('username_')) {
        ...
        if (action === 'create' && type === 'ssh') {
            userState[userId].step = `password_${action}_${type}`;
            await ctx.reply('🔑 *Masukkan password:*', { parse_mode: 'Markdown' });
        } else {
            // Langsung tampilkan pilihan durasi untuk create (non-ssh) dan renew
            const keyboard = {
                inline_keyboard: [
                    [{ text: '🥇1 Bulan', callback_data: `${action}_duration_${type}_${state.serverId}_1` }],
                    [{ text: '🥈2 Bulan', callback_data: `${action}_duration_${type}_${state.serverId}_2` }],
                    [{ text: '🥉3 Bulan', callback_data: `${action}_duration_${type}_${state.serverId}_3` }],
                    [{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]
                ]
            };
            await ctx.reply('⏳ *Pilih durasi masa aktif:*', {
    ...
    if (state && state.step.startsWith('password_')) {
        ...
        const keyboard = {
            inline_keyboard: [
                [{ text: '🥇1 Bulan', callback_data: `${action}_duration_${type}_${serverId}_1` }],
                [{ text: '🥈2 Bulan', callback_data: `${action}_duration_${type}_${serverId}_2` }],
                [{ text: '🥉3 Bulan', callback_data: `${action}_duration_${type}_${serverId}_3` }],
                [{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]
            ]
        };
        await ctx.reply('⏳ *Pilih durasi masa aktif:*', {
    ...
    if (state && state.step === 'input_kurangi_total_create') {
        const amountToReduce = parseInt(teks, 10);
        if (isNaN(amountToReduce) || amountToReduce <= 0) {
            return ctx.reply('❌ Masukkan angka yang valid dan lebih dari 0.');
        }

        db.run('UPDATE Server SET total_create_akun = CASE WHEN total_create_akun - ? < 0 THEN 0 ELSE total_create_akun - ? END WHERE id = ?', 
            [amountToReduce, amountToReduce, state.serverId], function(err) {
            if (err) {
                ctx.reply('❌ Gagal mengurangi total create akun.');
                logger.error(err.message);
            } else {
                ctx.reply(`✅ Total create akun berhasil dikurangi sebanyak ${amountToReduce}.`);
            }
            delete userState[userId];
        });
        return;
    ...
    if (state && state.step === 'edit_harga') {
        const hargaBulananStr = ctx.message.text.trim();
        const hargaBulanan = parseFloat(hargaBulananStr);

        if (isNaN(hargaBulanan) || hargaBulanan <= 0) {
            return ctx.reply('⚠️ *Harga bulanan tidak valid. Masukkan angka positif.*', { parse_mode: 'Markdown' });
        }

        try {
            const changes = await new Promise((resolve, reject) => {
                db.run('UPDATE Server SET harga = ? WHERE id = ?', [hargaBulanan, state.serverId], function(err) {
                    if (err) {
                        logger.error('⚠️ Kesalahan saat mengedit harga server:', err.message);
                        reject(err);
                    } else {
            ...
            if (changes > 0) {
                ctx.reply(`✅ *Harga server berhasil diubah menjadi Rp${hargaBulanan.toLocaleString('id-ID')}* per bulan.`, { parse_mode: 'Markdown' });
            } else {
...
bot.action('listserver', async (ctx) => {
  try {
    ...
    servers.forEach((server) => {
      // Harga dari DB sekarang adalah harga bulanan, tampilkan langsung
      const hargaBulanan = server.harga;

      serverList += `━━━━━━━━━━━━━━━━━━━━━━\n` +
                    `❇️ *${server.nama_server}*\n` +
                    `━━━━━━━━━━━━━━━━━━━━━━\n` +
                    `🏷️ » Harga Per Bulan: Rp${hargaBulanan.toLocaleString('id-ID')}\n` +                
                    `🚀 » Quota: ${server.quota} GB\n` +
                    `♂️ » Limit IP: ${server.iplimit} IP\n` +
...
// Aksi untuk menampilkan rekap SEMUA BULAN di TAHUN INI
bot.action('rekap_topup', async (ctx) => {
  await ctx.answerCbQuery('Mengumpulkan Data Dari Log Topup...');
  ...
  db.all(query, [currentYear], async (err, rows) => {
    if (err) {
      logger.error('❌ Gagal query rekap tahunan:', err.message);
      return ctx.reply('❌ Gagal mengambil data rekapitulasi.');
    }
    ...
    let message = `💰 *Rekap Total Top Up ${currentYear}*\n\n`;
    let grandTotal = 0;

    months.forEach((monthName, index) => {
      const total = monthlyTotals[index];
      grandTotal += total;
      message += `*${monthName}:* \`${formatRupiah(total)}\`\n`;
    });
    ...
    try {
      ...
      // Jika pesan terlalu tua untuk diedit, kirim pesan baru
...
async function sendReceiptToGroup(receipt, depositData) {
  // Cek apakah data receipt (PDF) valid
  if (!receipt || !receipt.success || !receipt.filePath) {
    logger.warn('Tidak ada data receipt PDF yang valid untuk dikirim ke grup.');
    return;
  }

  // Ambil info grup dari konfigurasi
  const groupData = getBotGroupData();
  if (!groupData || !groupData.chatId) {
    logger.warn('Group chat ID tidak dikonfigurasi, PDF tidak bisa dikirim.');
    return;
  }

  try {
    const caption = `✅ Top Up Sukses (via QRIS Orkut)\n\n` +
                  `👤 User: ${depositData.username || depositData.userId}\n` +
                  `💰 Nominal Dibayar: Rp${depositData.amount.toLocaleString('id-ID')}\n` +
                  `💵 Saldo Masuk: Rp${depositData.originalAmount.toLocaleString('id-ID')}\n\n` +
                  `Bukti pembayaran terlampir.`;

    // Mengirim dokumen dengan nama file yang benar
    await bot.telegram.sendDocument(
      groupData.chatId,
      {
        source: fs.createReadStream(receipt.filePath),
        filename: receipt.fileName 
      },
      { caption: caption }
    );

    logger.info(`✅ Bukti PDF untuk user ${depositData.userId} berhasil dikirim ke grup.`);

  } catch (error) {
    logger.error(`❌ Gagal mengirim bukti PDF ke grup: ${error.message}`);
  }
}
...
async function processDeposit(ctx, amount) {
  ...
  // Cegah spam permintaan QR
  if (currentTime - lastRequestTime < requestInterval) {
    await ctx.reply('⚠️ *Terlalu banyak permintaan. Silahkan tunggu sebentar sebelum mencoba lagi.*', {
      parse_mode: 'Markdown'
    });
    return;
  }
  ...
  try {
    ...
    // Generate QR
    const hasilGenerate = await qris.generateQR(finalAmount, uniqueCode);
    const qrBuffer = hasilGenerate.qrBuffer;

    if (!qrBuffer) {
      throw new Error('QR Buffer kosong.');
    }
    ...
    const inlineKeyboard = [
      [{ text: "📢 Join Channel", url: "https://t.me/pgetunnel" }],
    ...
    if (!qrMessage || !qrMessage.message_id) {
      throw new Error('Gagal kirim QR ke user.');
    }
    ...
    if (waitMsg) {
      try {
        ...
        logger.warn(`Gagal hapus waitMsg: ${e.message}`);
      }
    }
    ...
    // Simpan ke database
    await insertPendingDeposit(uniqueCode, userId, username, finalAmount, amount, qrMessage.message_id);

    // Hapus status input user
    if (global.depositState) delete global.depositState[userId];
    ...
    // Hanya hapus pendingDeposit kalau sudah sempat disimpan
    if (global.pendingDeposits && global.pendingDeposits[uniqueCode]) {
      delete global.pendingDeposits[uniqueCode];
      await deletePendingDeposit(uniqueCode);
    }

    await ctx.reply('❌ *GAGAL!* Terjadi kesalahan saat memproses pembayaran. Silahkan coba lagi nanti.', {
      parse_mode: 'Markdown'
    });
  }
}
...
async function sendTransactionLogToGroup({
  ...
  activeDays,
  costValue,
  hargaNormalPerBulan, // << UBAH NAMA PARAMETER
  saldoDikurangi,
  userSaldoNow,
  dateLabel,
  timeLabel
...
<b>╰──────────────────╯</b>
<b>╭ 🏷️No Trx:</b> #${trxNumber}
<b>┣ 🏷️Role:</b> <code>${statusText} ${statusEmoji}</code>
<b>┣ 🏷️Username:</b> ${tgUsername}
<b>┣ 🏷️ID:</b> <code>${tgUserId}</code>
<b>┣ 🏷️Server:</b> ${serverName}
<b>┣ 🏷️ISP :</b> ${ispName}
<b>┣ 🏷️Domain / IP :</b> <code>${domainName}</code>
<b>┣ 🏷️Nama :</b> ${censoredAccountUsername}
<b>┣ 🏷️Produk:</b> ${serviceName}
<b>┣ 🏷️Limit Quota :</b> ${limitQuota} GB
<b>┣ 🏷️Limit Login :</b> ${limitLogin} Hp
<b>┣ 🏷️Type:</b> ${trxType}
<b>┣ 🏷️Durasi akun:</b> ${activeDays} Hari Rp.${costValue.toLocaleString('id-ID')}
<b>┣ 🏷️Harga Normal Perbulan:</b> Rp.${hargaNormalPerBulan.toLocaleString('id-ID')}
<b>┣ 🏷️Saldo di kurangi:</b> Rp.${saldoDikurangi.toLocaleString('id-ID')}
<b>┣ 🏷️Saldo saat ini:</b> Rp.${userSaldoNow.toLocaleString('id-ID')}
<b>┣ 🏷️Tanggal:</b> ${dateLabel}
<b>┣ 🏷️Waktu:</b> ${timeLabel}
<b>╰──────────────────╯</b>
`;
...
async function afterAccountTransaction({
  ...
  try {
    ...
    const hargaBulanan = serverDetails.harga || 0; // Harga dari DB adalah harga bulanan
    ...
    const jumlahBulan = durasi / 30; // Hitung jumlah bulan (misal: 30 hari / 30 = 1 bulan)
    let totalHarga = hargaBulanan * jumlahBulan; // Kalikan harga bulanan dengan jumlah bulan
    ...
    const waktu = now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }).replace(/\./g, ':') + ' WIB';
    ...
    await sendTransactionLogToGroup({
      ...
      tgUserId: userId,
      serverName: serverDetails.nama_server || '-',
      ispName: ispServer,
      ...
      hargaNormalPerBulan: hargaBulanan || 0,
...
async function sendPaymentSuccessNotificationByUserId(
  ...
  try {
    ...
    const statusText =
      adminIds.includes(userId) ? 'Admin 🌀' :
      userRole === 'reseller'   ? 'Reseller 🔑' :
                                  'Member 📌';

    // Helper format Rupiah
    const rup = n => (n || 0).toLocaleString('id-ID');

    // Siapkan blok teks bonus & saldo ditambahkan (dinamis)
    const hasBonus = deposit.bonus && deposit.bonus > 0;
    const bonusBlock = hasBonus
      ? `🎁 *» Bonus Top Up:* Rp${rup(deposit.bonus)} (${deposit.bonus_percent}%)\n` +
        `🏷️ *» Saldo Ditambahkan:* Rp${rup(deposit.originalAmount)} + Rp${rup(deposit.bonus)}\n`
      : `🏷️ *» Saldo Ditambahkan:* Rp${rup(deposit.originalAmount)}\n`;

    const messageText =
      `╭──────────────────╮\n` +
      `❇️    *Pembayaran Berhasil*      ❇️\n` +
      `╰──────────────────╯\n` +
      `╭──────────────────╮\n` +
      `🏷️ *» Role:* \`${statusText}\`\n` +
      `🏷️ *» Username:* @${username}\n` +
      `🏷️ *» ID Pengguna:* \`${userId}\`\n` +
      `🏷️ *» Nominal:* Rp${rup(deposit.amount)}\n` +
      bonusBlock +
      `🏷️ *» Saldo Sekarang:* Rp${rup(currentBalance)}\n` +
      `╰──────────────────╯`;
